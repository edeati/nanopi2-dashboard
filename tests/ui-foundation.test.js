...
module.exports = async function run() {
  ...
  assert.ok(html.indexOf('id="solarIsoChip"') > -1, 'solar isolation chrome chip missing');
  assert.ok(html.indexOf('id="solarInvChip"') > -1, 'solar inverter chrome chip missing');
  assert.ok(html.indexOf('function updateSolarChartChrome(') > -1, 'solar chart chrome updater missing');
  ...
  assert.ok(html.indexOf("solarChartImage.src = '/api/solar/chart.svg?rev=6&mode=' + (ready ? 'ready' : 'pending') + '&slot=' + (bucket % 2) + '&iso=' + isoPoints;") > -1, 'solar chart should alternate bounded no-store URLs');
