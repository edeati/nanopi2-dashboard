...
module.exports = async function run() {
  ...
  assert.ok(html.indexOf('height: 96px;') > -1, 'solar chart canvas should be taller');
  ...
  assert.ok(html.indexOf('min-height: 54px;') > -1, 'solar daily cards should be larger');
  ...
  assert.ok(html.indexOf('font-size: clamp(13px, 2vh, 16px);') > -1, 'weather summary should be compact to avoid clipping');
  assert.ok(html.indexOf('font-size: clamp(24px, 3.8vh, 34px);') > -1, 'weather temperature should be compact to avoid clipping');
  ...
  assert.ok(html.indexOf('font-size: 44px;') > -1, 'bin icon should be larger for distance readability');
  assert.ok(html.indexOf('id="binDetails"') > -1, 'bin details wrapper missing');
  assert.ok(html.indexOf('#binCard { display: grid; grid-template-columns: auto 1fr;') > -1, 'bin card should place text to the right of icon');
  ...
  assert.ok(html.indexOf('object-fit: contain;') > -1, 'radar gif should render uncropped');
