...
module.exports = async function run() {
  ...
  assert.ok(html.indexOf('.strip-card.strip-card-bin.strip-tone-yellow .strip-card-bg-icon {\n      color: #ffcc00;') > -1, 'recycle bin icon should be yellow');
  assert.ok(html.indexOf('.strip-card.strip-card-bin .strip-card-bg-icon.is-hero {\n      opacity: 0.72;') > -1, 'bin hero background icon should be more visible');
  assert.ok(html.indexOf('grid-template-rows: auto minmax(0, 1fr) max-content max-content max-content minmax(0, 1fr);') > -1, 'bin strip cards should keep the kicker at the top and center the main content');
  ...
  assert.ok(html.indexOf('grid-template-columns: minmax(0, 1fr) 210px 180px;') > -1, 'tablet bottom row should give the static pool and clock cards readable width');
  ...
  assert.ok(html.indexOf('#timeBig {\n        font-size: 62px;') > -1, 'tablet clock should be larger while fitting beside the pool cleaner card');
  ...
  assert.ok(html.indexOf('id="solarUsageChartImage"') > -1, 'backend-rendered solar chart image missing');
  assert.ok(html.indexOf('src="/api/solar/chart.svg') === -1, 'solar chart URL should be versioned at runtime');
  assert.strictEqual(html.indexOf('id="solarDawnChart"'), -1, 'hidden dawn canvas should be removed');
  ...
  assert.ok(html.indexOf('height: 196px;') > -1, 'solar chart panel should stay 196px so status cards are not pushed');
  assert.ok(html.indexOf('.chart {\n      width: 100%;\n      /* Leave top/bottom black gutters inside the panel for badge divs (1024x768) */\n      height: 128px;') > -1, 'solar chart image should leave black gutters for overlay badges');
  ...
  assert.ok(html.indexOf('body.takeover-solar .chart {\n      height: 320px;') > -1, 'solar chart canvas should leave black gutters for badges in takeover mode');
  ...
  assert.ok(html.indexOf('id="solarIsoOverlay"') > -1, 'isolation overlay badge missing inside chart-wrap');
  assert.ok(html.indexOf('.solar-chart-overlay .ov-value {\n      font-family: var(--font-display);\n      font-size: 14px;') > -1, 'chart overlay values should stay compact relative to the solar chart on 1024x768');
  assert.ok(html.indexOf('id="solarInvOverlay"') > -1, 'inverter overlay badge missing inside chart-wrap');
  assert.ok(html.indexOf('function updateSolarChartOverlays(') > -1, 'solar chart overlay updater missing');
  assert.ok(html.indexOf('function describeInverterCode(') > -1, 'inverter code classifier missing');
  assert.ok(html.indexOf("err === 306") > -1, '306 PV power low should be informational');
  assert.ok(html.indexOf("err === 307") > -1, '307 DC voltage too low should be informational');
  assert.ok(html.indexOf("err === 509") > -1, '509 no feed-in should be informational');
  assert.ok(html.indexOf("err === 522 || err === 523") > -1, '522/523 DC input voltage too low should be informational');
  assert.ok(html.indexOf('.solar-chart-overlay') > -1, 'solar chart overlay styles missing');
  ...
  assert.ok(html.indexOf("solarChartImage.src = '/api/solar/chart.svg?rev=14&mode=' + (ready ? 'ready' : 'pending') + '&slot=' + (bucket % 2) + '&iso=' + isoPoints + '&err=' + (isFinite(isoErr) ? isoErr : 0);") > -1, 'solar chart should alternate bounded no-store URLs');
  ...
  assert.strictEqual(html.indexOf('function drawDawnQuarterBars('), -1, 'hidden dawn canvas renderer should be removed');
  assert.ok(html.indexOf('if (canvas.width !== nextWidth) {') > -1, 'canvas resize should preserve unchanged backing stores');
  assert.ok(html.indexOf('if (canvas.height !== nextHeight) {') > -1, 'canvas resize should avoid unnecessary GPU buffer allocation');
  ...
  assert.ok(html.indexOf('if (flowSig !== lastSolarFlowSig) {') > -1, 'flow panel redraw guard missing');
  ...
  assert.ok(html.indexOf('setTimeout(function () {\n          applyViewMode(viewMode, untilMs, nextAtMs);\n        }, TAKEOVER_BLACKOUT_MS);') > -1, 'fullscreen transition should delay in-page view switch to show blackout');
  assert.ok(html.indexOf('transitionToView(mode === \'radar\' ? \'radar\' : \'solar\', now + durationMs, nextAtMs);') > -1, 'takeover start should use in-page blackout transition');
  assert.ok(html.indexOf('transitionToView(\'main\', 0, nextAtMs);') > -1, 'takeover end should use in-page blackout transition');
  assert.ok(html.indexOf("var lastFocusMode = '';") > -1, 'focus rotation should persist last mode across reloads');
  assert.strictEqual(html.indexOf("lastFocusMode = normalizeFocusMode(search.get('lastfocus'));"), -1, 'initial view parsing should no longer restore focus mode from URL');
  assert.strictEqual(html.indexOf("url.searchParams.set('lastfocus', lastFocusMode);"), -1, 'view mode should not carry focus state in the URL');
